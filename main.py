...
@app.post("/api/game/start-drinking")
def start_drinking(request: StartDrinkingRequest):
    """開始飲酒階段 - 觸發幫浦並開始倒數"""
    if not game_room.game_started:
        raise HTTPException(status_code=400, detail="遊戲尚未開始")

    game_room.drinking_player_id = request.player_id
    game_room.drinking_player_name = request.player_name
    game_room.drinking_start_time = datetime.now()

    print(f"🍺 開始飲酒階段: {request.player_name} (ID: {request.player_id})")

    return {
        "success": True,
        "message": "開始飲酒",
        "drinking_player_name": request.player_name
    }

@app.post("/api/game/finish-drinking")
def finish_drinking(request: HeartbeatRequest):
    """結束飲酒階段 - 玩家確認已喝完"""
    if not game_room.game_started:
        raise HTTPException(status_code=400, detail="遊戲尚未開始")

    player_name = game_room.drinking_player_name or "玩家"

    game_room.drinking_player_id = None
    game_room.drinking_player_name = None
    game_room.drinking_start_time = None

    print(f"✅ 飲酒完成: {player_name}")
...
def reset_game():
    ...
    try:
        ...
        game_room.mode_selected = False # 允許重新選擇模式
        game_room.game_ended = False
        game_room.game_result = None
...
def game_event(request: GameEventRequest):
    ...
    if(request.pump_id != -1):
        decision = resolve_game_event(
            mode=request.mode,
            event=request.event,
            score=request.score,
            base_pump_id=request.pump_id
        )
    else:
        decision = resolve_game_event(
            mode=request.mode,
...
@app.post("/api/game/set-event")
def set_event(request: SetEventRequest):
    """設定當前顯示的彈窗事件"""
    game_room.active_event = {
        "id": str(uuid.uuid4()), # 產生唯一 ID 確保前端知道這是新事件
        "title": request.title,
        "body": request.body,
...
@app.post("/api/game/clear-event")
def clear_event():
    """清除當前彈窗"""
    game_room.active_event = None
    return {"success": True}
# =========================================================
...
class ColorPumpRequest(BaseModel):
    color: str  # red, blue, yellow, green
    duration: Optional[float] = 1.0

@app.post("/api/debug/pump-by-color")
def pump_by_color(request: ColorPumpRequest):
    """Debug 用：根據顏色啟動對應的幫浦"""
    if request.color not in COLOR_TO_PUMP:
        raise HTTPException(status_code=400, detail=f"無效顏色: {request.color}")

    pump_id = get_pump_for_color(request.color)

    try:
        pump_controller.pump_out(pump_id, request.duration)
        return {
            "success": True,
            "color": request.color,
            "pump_id": pump_id,
            "duration": request.duration,
            "message": f"顏色 {request.color} → 幫浦 {pump_id} 已運行 {request.duration} 秒"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
