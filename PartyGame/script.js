    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            if (data.status === 'in_queue') {
                ...
                // 如果是房主且還未選擇模式，設定模式到後端
                if (data.room_state.is_host && !data.room_state.mode_selected) {
                    const selectedMode = sessionStorage.getItem('cheers_mode');
                    if (selectedMode) {
                        console.log('🎮 房主設定模式:', selectedMode);
                        try {
                            const modeResponse = await fetch('/api/room/set-mode', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                credentials: 'include',
                                body: JSON.stringify({
                            ...
                            if (modeResponse.ok && modeData.success) {
                                console.log('✅ 模式設定成功:', modeData.game_mode);
                            } else {
                                console.error('❌ 模式設定失敗:', modeData);
                            }
                        } catch (error) {
...
async function pollRoomState() {
    try {
        ...
        if (state.mode_selected && state.game_mode) {
            const localMode = sessionStorage.getItem('cheers_mode');
            if (localMode !== state.game_mode) {
                console.log('🔄 同步後端模式:', state.game_mode, '(本地:', localMode, ')');
                sessionStorage.setItem('cheers_mode', state.game_mode);
            }
        }
...
// 飲酒倒數狀態
let drinkingInterval = null;
// let drinkingCallback = null;  // ✅ 已棄用：改為直接在 finishDrinking() 中執行換基底邏輯

// 題目現在從後端 API 獲取，不再硬編碼

// --- 飲酒階段函式 ---
async function startDrinkingPhase(playerIndex) {
    /**
     * 開始飲酒階段
     * @param {number} playerIndex - 飲酒玩家的索引
     */
    ...
    // 通知後端開始飲酒（觸發幫浦）
    try {
        await fetch('/api/game/start-drinking', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({
        ...
        // 顯示飲酒倒數 modal
        showDrinkingModal(playerName, player.player_id);
    } catch (error) {
        console.error('❌ 開始飲酒階段失敗:', error);
    }
}

function showDrinkingModal(playerName, drinkingPlayerId, initialCountdown = 30) {
    /**
     * 顯示飲酒倒數 modal
     * @param {string} playerName - 飲酒玩家名字
     * @param {string} drinkingPlayerId - 飲酒玩家 ID
     * @param {number} initialCountdown - 初始倒數秒數（預設 30，支援從後端同步）
     */
    const modal = document.getElementById('drinkingModal');
    const nameEl = document.getElementById('drinkingPlayerName');
    const countdownEl = document.getElementById('drinkingCountdown');
    const doneBtn = document.getElementById('drinkingDoneBtn');

    nameEl.textContent = playerName;
    countdownEl.textContent = initialCountdown;

    // ✅ 確保 myPlayerId 已設定（如果未定義，嘗試從 cookie 恢復）
    if (!myPlayerId) {
        console.warn('⚠️ myPlayerId 未定義，嘗試從 cookie 恢復');
        const cookies = document.cookie.split(';');
        for (let cookie of cookies) {
            const [key, value] = cookie.trim().split('=');
            if (key === 'player_id') {
    ...
    // ✅ 詳細日誌：追蹤按鈕邏輯判斷
    console.log('🔍 [Bug #9 偵錯] 飲酒 Modal 按鈕邏輯');
    console.log('   - 飲酒玩家名稱:', playerName);
    console.log('   - drinkingPlayerId:', drinkingPlayerId, '(type:', typeof drinkingPlayerId, ')');
    console.log('   - myPlayerId:', myPlayerId, '(type:', typeof myPlayerId, ')');
    console.log('   - currentPlayerIndex:', currentPlayerIndex);
    console.log('   - 當前回合玩家ID:', gamePlayers[currentPlayerIndex]?.player_id);

    // 只有飲酒玩家可以點擊「喝完了」按鈕
    // ✅ 確保比較時都轉換為字串（避免類型不一致）
    const drinkingPlayerIdStr = drinkingPlayerId ? String(drinkingPlayerId) : null;
    const myPlayerIdStr = myPlayerId ? String(myPlayerId) : null;

    const isMyDrink = drinkingPlayerIdStr && myPlayerIdStr && (drinkingPlayerIdStr === myPlayerIdStr);

    console.log('   - 比較結果 (drinkingPlayerIdStr === myPlayerIdStr):', drinkingPlayerIdStr === myPlayerIdStr);
    console.log('   - isMyDrink:', isMyDrink);

    doneBtn.disabled = !isMyDrink;
    doneBtn.style.opacity = isMyDrink ? '1' : '0.5';
    doneBtn.textContent = isMyDrink ? '喝完了!' : '等待中...';
    ...
    drinkingInterval = setInterval(() => {
        countdown--;
        countdownEl.textContent = countdown;
        if (countdown <= 0) {
            clearInterval(drinkingInterval);
...
async function finishDrinking() {
    ...
    if (drinkingInterval) {
        clearInterval(drinkingInterval);
        drinkingInterval = null;
    }

    // 隱藏 modal
    document.getElementById('drinkingModal').classList.add('hidden');

    // 通知後端飲酒完成
    try {
        await fetch('/api/game/finish-drinking', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({ player_id: myPlayerId })
        ...
        console.error('❌ 完成飲酒通知失敗:', error);
    }

    // ✅ 直接執行換基底邏輯（不再依賴 callback）
    // 檢查遊戲是否已結束
    if (currentGameMode === 'family' && currentRound >= 6) {
        console.log('🏁 闔家歡模式已結束（已完成 5 回合），不再倒酒');
        nextTurn();
        return;
    }
    // 檢查酒鬼模式是否有人達到失敗條件
    if (currentGameMode === 'drunk' && gamePlayers.some(p => p.score >= LOSE_THRESHOLD)) {
        console.log('🏁 酒鬼模式已結束，不再倒酒');
        nextTurn();
        return;
    }

    // 喝完後才執行以下動作
    console.log('🍺 玩家喝完了，準備換新基底並倒酒...');

    // 1️⃣ 先換基底（只改狀態）
    await resetBaseWine();

    // 2️⃣ 再真正倒酒（幫浦）
    await pourBaseWine(null, 'after_drink');

    // 3️⃣ 下一回合 - ✅ 加上 await 確保完全切換完成
    await nextTurn();
}
...
function startSyncedWheelSpin(seed, winnerIndex) {
    ...
    console.log('🎰 啟動平滑同步轉盤，種子:', seed, '中獎索引:', winnerIndex);
    ...
    // 1. 設定動畫時間 (4~7秒)
    const seededRandom = seed / 10000;
    spinTime = 0;
    spinTimeTotal = seededRandom * 3000 + 4000;

    // 2. 計算目標角度 (解決指針不準的問題)
    // Canvas 0度在右邊(3點鐘)，指針在上面(270度/1.5PI)
    // 我們希望 winnerIndex 這個扇形的中心點最終停在 1.5PI
    const targetAngleRad = (1.5 * Math.PI) - (winnerIndex * arc) - (arc / 2);

    // 3. 計算需要轉多遠 (解決跳動問題)
    // 讓我們目前的角度 (startAngle) 加上 "幾圈" 後，剛好落在 targetAngleRad
    const currentAngle = startAngle;
    
    // 確保目標角度在當前角度之後 (往未來轉)，計算出最小正差距
    // 這裡使用 (target - current) % 2PI 的邏輯，並確保結果為正
    let diff = targetAngleRad - currentAngle;
    while (diff < 0) {
        diff += (2 * Math.PI);
    }
    
    // 加上額外的旋轉圈數 (至少 5 圈，增加刺激感，並由種子決定圈數)
    const extraSpins = 5 + Math.floor(seededRandom * 5); // 5~10 圈
    const totalRotation = diff + (extraSpins * 2 * Math.PI);

    // 4. 儲存動畫參數到全域變數 (供 rotateSyncedWheel 使用)
    window.spinStartAngle = currentAngle;       // 起點
    window.spinTotalChange = totalRotation;     // 總共要跑的距離 (終點 - 起點)
    window.syncedWinnerIndex = winnerIndex;

    console.log('🎯 動畫規劃:', {
        起點: window.spinStartAngle,
        總距離: window.spinTotalChange,
        目標索引: winnerIndex
    });
...
function rotateSyncedWheel() {
    ...
    // 如果時間到了
    if (spinTime >= spinTimeTotal) {
        // 強制設定為精確的終點角度 (避免最後一幀的微小誤差)
        startAngle = window.spinStartAngle + window.spinTotalChange;
        drawWheelWithNames();
        stopSyncedWheel();
        return;
    }
    ...
    // 使用緩動函數計算 "當前應該在哪個角度"
    // t: 當前時間, b: 起始值, c: 變化量, d: 總時間
    const currentChange = easeOut(spinTime, 0, window.spinTotalChange, spinTimeTotal);
    
    // 更新當前角度
    startAngle = window.spinStartAngle + currentChange;
...
// 停止轉盤並通知後端完成
// 修改重點：移除強制跳轉邏輯，因為動畫已經準確到達
async function stopSyncedWheel() {
    clearTimeout(spinTimeout);
    isSpinning = false;

    // 這裡不需要再計算 normalizedTarget 並強制賦值了
    // 因為上面的動畫已經保證 startAngle 停在最完美的位置

    console.log('🎰 轉盤動畫完成，位置完美，通知後端...');
        ...
        if (response.ok) {
            ...
            if (!state.base_wine_color) {
                ...
                if (isHost) {
                    ...
                    await resetBaseWine();
                    await pourBaseWine(null, 'game_start');  // 新增：實際倒出基底酒
                } else {
...
async function pollGameState() {
    try {
        ...
        // 同步飲酒狀態（所有玩家看到飲酒倒數 modal）
        if (state.drinking_player_id) {
            const drinkingModal = document.getElementById('drinkingModal');
            if (drinkingModal && drinkingModal.classList.contains('hidden')) {
                // 有玩家正在飲酒 - 顯示 modal
                console.log(`🍺 同步飲酒狀態: ${state.drinking_player_name} 正在飲酒`);

                // ✅ 計算剩餘時間（從後端同步），傳入 showDrinkingModal
                let initialCountdown = 30; // 預設值
                if (state.drinking_start_time) {
                    const startTime = new Date(state.drinking_start_time);
                    const elapsed = Math.floor((Date.now() - startTime.getTime()) / 1000);
                    initialCountdown = Math.max(0, state.drinking_duration - elapsed);
                    console.log(`⏱️ 從後端同步倒數時間: ${initialCountdown} 秒 (已過 ${elapsed} 秒)`);
                }

                // 傳入計算好的倒數值
                showDrinkingModal(state.drinking_player_name, state.drinking_player_id, initialCountdown);
            }
        } else {
            // 沒有玩家在飲酒 - 隱藏 modal（如果可見的話）
            const drinkingModal = document.getElementById('drinkingModal');
            if (drinkingModal && !drinkingModal.classList.contains('hidden')) {
                console.log('✅ 飲酒完成，隱藏 modal');
                drinkingModal.classList.add('hidden');
                if (drinkingInterval) {
                    clearInterval(drinkingInterval);
                    drinkingInterval = null;
                }
...
// 同步全域彈窗事件 (讓所有人都看到黑白切/LSA等)
        if (state.active_event) {
            // 如果是新事件 (ID 不同)，才開啟 Modal
            if (state.active_event.id !== lastProcessedEventId) {
                console.log('🔔 收到同步事件:', state.active_event.title);
                lastProcessedEventId = state.active_event.id;
                
                // [修改] 傳入 target_player_id 給 openModalLocal
                openModalLocal(
                    state.active_event.title, 
                    state.active_event.body, 
                    state.active_event.actions,
                    state.active_event.target_player_id // [新增] 從後端狀態取得目標 ID
                );
            }
        } else {
            // [重要] 這裡要保留，否則視窗不會自動關閉
            // 如果後端沒有事件，且目前有開啟 Modal，則關閉
...
async function handleDrinkAndResetFamily() {
    ...
    // 先顯示飲酒 modal，等玩家喝完後才重置基底並觸發幫浦
    setTimeout(() => {
        startDrinkingPhase(currentPlayerIndex);
    }, 1500);
}
...
function handleBlackWhiteFamily() { let opponentIndices = gamePlayers.map((_, i) => i).filter(i => i !== currentPlayerIndex); currentOpponentIndex = opponentIndices[Math.floor(Math.random() * opponentIndices.length)]; let opponent = gamePlayers[currentOpponentIndex]; const targetId = gamePlayers[currentPlayerIndex].player_id; openModal("黑白切對決！", `<p>對手是：<strong style="color:var(--neon-pink)">${opponent.name}</strong></p><p>快去實體對決！誰贏了？</p>`, `<button class="option-btn" onclick="resolveBW(true)">我贏了 (我+1, 對手-1)</button><button class="option-btn" onclick="resolveBW(false)">我輸了 (我-1, 對手+1)</button>`, 'broadcast', targetId); }
...
function handleDragonGate() {
    ...
    // 使用 openModal 的 targetPlayerId 参数来锁定操作权限
    openModal(
        "射龍門",
        `<div style="text-align:center;">
            <div style="margin-bottom:10px;">龍柱</div>
            <div>
                <span class="poker-card ${[1,13].includes(c1)?'red':''}">${getCardDisplay(c1)}</span>
                <span class="poker-card ${[1,13].includes(c2)?'red':''}">${getCardDisplay(c2)}</span>
            </div>
            <div style="margin:20px 0; font-size:1.2rem;">
                第三張牌需在 <span style="color:var(--neon-blue)">${minC} ~ ${maxC}</span> 之間 (不含)
            </div>
            <div id="dragonResultArea" style="min-height:140px; display:flex; justify-content:center; align-items:center;">
                <button id="draw3rdBtn" class="neon-btn" style="padding:10px 30px; margin:0;" onclick="drawDragonCard(${minC}, ${maxC})">抽第三張</button>
            </div>
        </div>`,
        "",
        'broadcast',
        currentPlayerId  // 只有当前玩家可以点击
    );
}
...
window.drawDragonCard = function(min, max) {
    const c3 = Math.floor(Math.random() * 13) + 1;
    const isSafe = (c3 > min && c3 < max);
    ...
    const resultHtml = `
        <div style="display:flex; flex-direction:column; align-items:center;">
            <div style="margin-bottom:10px; color:#aaa;">範圍: ${min} ~ ${max}</div>
            <span class="poker-card ${[1,13].includes(c3)?'red':''}">${getCardDisplay(c3)}</span>
            <div style="margin-top:10px; font-weight:bold; font-size:1.5rem; color:${isSafe ? '#00ff00' : 'red'}">
                ${isSafe ? '過關！' : '撞柱/射偏！喝！'}
            </div>
            <button class="option-btn" style="margin-top:15px;" onclick="closeDragonModal(${isSafe})">確定</button>
        </div>
    `;

    // [修改重點] 再次呼叫 openModal，將結果同步給所有觀看者，并锁定只有当前玩家可以点击"确定"
    openModal("射龍門 - 結果", resultHtml, "", 'broadcast', currentPlayerId);
}
...
async function handleLSAQuestion() {
    try {
        ...
        const targetId = gamePlayers[currentPlayerIndex].player_id;
        openModal("LSA 知識大考驗", `<p style="margin-bottom:10px;">${qData.question}</p>`, actions, 'broadcast', targetId);
    } catch (error) {
...
async function handleTruthOrDare() {
    try {
        ...
        const targetId = gamePlayers[currentPlayerIndex].player_id;
        openModal(`真心話大冒險 - ${title}`, `<div style="font-size: 1.5rem; font-weight: bold; color: var(--neon-pink); margin-bottom: 20px;">${selectedQuestion}</div>`, `<button class="option-btn" onclick="closeModal(); nextTurn();">挑戰完成 (下一位)</button>`, 'broadcast', targetId);
    } catch (error) {
...
function getColorNum(key) { if(key === 'red') return '1'; if(key === 'blue') return '2'; if(key === 'yellow') return '3'; return '4'; }
...
// [修改] 開啟 Modal 的統一入口
// behavior: 'broadcast' (預設，通知所有人), 'local' (接收端)
// targetPlayerId: 指定誰可以操作按鈕，預設為 null (代表發送者自己)
async function openModal(title, bodyHtml, actionsHtml, behavior = 'broadcast', targetPlayerId = null) {
    
    // [修正 1] 計算實際的操作者 ID
    // 如果是廣播模式且沒指定人，預設就是我自己 (myPlayerId)
    const effectiveTargetId = (behavior === 'broadcast' && !targetPlayerId) ? myPlayerId : targetPlayerId;

    // 1. 本地先顯示 (傳入 effectiveTargetId 以供判斷是否鎖定按鈕)
    openModalLocal(title, bodyHtml, actionsHtml, effectiveTargetId);

    // 2. 如果是主動觸發，通知後端同步給所有人
    if (behavior === 'broadcast') {
        try {
            await fetch('/api/game/set-event', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    title: title,
                    body: bodyHtml,
                    actions: actionsHtml,      // [修正 2] 補上逗號
                    target_player_id: effectiveTargetId // [修正 3] 傳送正確的 ID
                })
            });
        } catch (e) {
            console.error('廣播事件失敗', e);
        }
    }
}

// [新增] 純本地開啟 Modal
/*function openModalLocal(title, bodyHtml, actionsHtml) {
...
// [修改] 純本地開啟 Modal
function openModalLocal(title, bodyHtml, actionsHtml, targetPlayerId = null) {
    document.getElementById("modalTitle").innerText = title;
    document.getElementById("modalBody").innerHTML = bodyHtml;
    ...
    // [修改] 權限檢查邏輯 (升級版：支援 button 和 div 選項)
    // 如果有指定目標玩家，且目標玩家不是我，則禁用所有互動元件
    if (targetPlayerId && targetPlayerId !== myPlayerId) {
        console.log(`🔒 鎖定操作：目標(${targetPlayerId}) !== 我(${myPlayerId})`);
        
        // [關鍵修改] 同時選取 button 和 .color-choice-btn (顏色選項)
        const interactives = document.querySelectorAll('#modalBody button, #modalBody .color-choice-btn, #modalActions button, #modalActions .color-choice-btn');
        
        interactives.forEach(el => {
            el.disabled = true;              // 對 button 有效
            el.style.pointerEvents = 'none'; // [關鍵] 對 div 有效 (讓 onclick 失效)
            el.style.opacity = '0.5';        // 讓外觀變半透明
            el.style.cursor = 'not-allowed';
            
            // 選擇性：修改文字提示
            // 只對一般按鈕修改文字，避免破壞顏色選項(div)的樣式
            if (el.tagName === 'BUTTON' && !el.getAttribute('onclick').includes('closeModalLocal')) {
...
window.closeModal = async function() {
    closeModalLocal();
    try {
        await fetch('/api/game/clear-event', { method: 'POST' });
...
function showRules() { 
    if (currentGameMode === 'family') { 
        openModal(
            "闔家歡模式遊戲規則", 
            `<div style="text-align: left;"><p style="color:var(--neon-yellow); font-weight:bold; margin-bottom:5px;">1. 基底酒機制</p><ul class="rules-list"><li>電腦隨機選色作為基底。</li><li><strong>結算：</strong>每 5 回合結算。</li></ul><p style="color:var(--neon-yellow); font-weight:bold; margin-bottom:5px;">2. 骰子事件表</p><table class="rules-table"><thead><tr><th width="30%">點數</th><th>事件說明</th></tr></thead><tbody><tr><td>3 或 5</td><td>LSA 選擇題</td></tr><tr><td>4 或 8</td><td>電腦加酒</td></tr><tr><td>6</td><td>黑白切</td></tr><tr><td>7</td><td>自選加酒</td></tr><tr><td>9 或對子</td><td>喝酒 (積分+1)</td></tr><tr><td>10 或 11</td><td>真心話大冒險</td></tr></tbody></table></div>`, 
            `<button class="option-btn" onclick="closeModalLocal()">了解</button>`, // 修改：只關閉本地視窗
            'local' // 修改：強制設定為本地模式，不廣播給其他人
        ); 
    } else { 
        openModal(
            "無乃酒鬼模式規則", 
            `<div style="text-align: left;"><p style="color:var(--neon-yellow); font-weight:bold; margin-bottom:5px;">模式核心</p><ul class="rules-list"><li><strong>無回合限制</strong>，直到有人喝滿 3 杯。</li></ul><p style="color:var(--neon-yellow); font-weight:bold; margin-bottom:5px;">骰子事件表</p><table class="rules-table"><thead><tr><th width="30%">點數</th><th>事件說明</th></tr></thead><tbody><tr><td>3</td><td>我有你沒有</td></tr><tr><td>4 或 8</td><td>電腦加酒</td></tr><tr><td>5</td><td>掰手腕</td></tr><tr><td>6</td><td>黑白切</td></tr><tr><td>7</td><td>自選加酒</td></tr><tr><td>9</td><td>射龍門</td></tr><tr><td>對子</td><td>直接喝一杯！</td></tr><tr><td>10 或 11</td><td>真心話大冒險</td></tr></tbody></table></div>`, 
            `<button class="option-btn" onclick="closeModalLocal()">了解</button>`, // 修改：只關閉本地視窗
            'local' // 修改：強制設定為本地模式，不廣播給其他人
        ); 
    } 
